...
{   
	 ...
	 * ServerKeyExchange signature algorithm. Without it, strict TLS 1.2
	 * servers send handshake_failure. We advertise SHA-256+RSA and SHA-1+RSA.
	 */
	if(clientHello->protocolVersion >= TLS_Version_1_2) {
		/* type(2) + ext_len(2) + list_len(2) + 2 pairs * 2 bytes each = 12 */
		sigAlgsLen = 2 + 2 + 2 + (2 * 2);
    ...
    *p++ = 0;                               /* null compression */
	...
	if(sigAlgsLen) {
		...
		/* extension data length = list_len(2) + 2 pairs * 2 = 6 */
		p = SSLEncodeInt(p, 6, 2);
		/* supported_signature_algs list length in bytes = 2 pairs * 2 = 4 */
		p = SSLEncodeInt(p, 4, 2);
		/* SHA-256 (4) + RSA (1) */
		p = SSLEncodeInt(p, 4, 1);
		p = SSLEncodeInt(p, 1, 1);
		/* SHA-1 (2) + RSA (1) */
		p = SSLEncodeInt(p, 2, 1);
		p = SSLEncodeInt(p, 1, 1);
