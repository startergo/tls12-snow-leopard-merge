...
//
template <class Type>
static inline void h2ni(Type &v)	{ v = Type(h2n(v)); }
...
public:
    ...
    Endian(Value v) : mValue(Value(h2n(v))) { }
    
    operator Value () const		{ return Value(n2h(mValue)); }
    Endian &operator = (Value v)	{ mValue = Value(h2n(v)); return *this; }
